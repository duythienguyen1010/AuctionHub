...
exports.validateGreaterThan1 = (req, res, next) => {
    let price = req.body.initialPrice;
    if (price >= 1) {
        return next();
    } else {
        let err = new Error("Minimum starting price is $1");
        err.status = 400;
        return next(err);
    }
};
...
expirationDate = exports.validateTrade = [
    ...
    body("description", "Description must be at least 10 characters long")
        ...
        .trim()
        .escape(),
    body("initialPrice", "Starting price is required")
        .notEmpty()
        .trim()
        .escape(),
];
